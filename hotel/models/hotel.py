...
class hotel_folio(models.Model):
    ...
    currency_id = fields.Many2one('res.currency', 'Currency', required=True, related='pricelist_id.currency_id',
                                  default=lambda self: self.pricelist_id.currency_id.id)
...
class hotel_folio_line(models.Model):
    ...
    def write(self, vals):
        for line_obj in self:
            # Search for the related reservation line
            line_search = self.env['hotel.reservation.line'].search(
                [('id', '=', line_obj.hotel_reservation_line_id.id)]
            )

            # Check if the product_id has changed in vals
            if 'product_id' in vals:
                # Loop through each room in the room_number field (many2many)
                for room in line_search.room_number:
                    if room.id != vals['product_id']:
                        room_id_room_data = self.env['hotel.room'].search(
                            [('name', '=', room.name)]
                        )
                        # Check each history record in room_folio_ids
                        for history in room_id_room_data.room_folio_ids:
                            if (history.booking_id.id == line_search.line_id.id
                                    and line_obj.checkin_date == history.check_in
                                    and line_obj.checkout_date == history.check_out):
                                history.unlink()

                        # Get the new room name for the updated product_id
                        room_name = self.env['hotel.room'].search(
                            [('product_id', '=', vals['product_id'])]
                        )

                        # Prepare history record values
                        value_history = {
                            'partner_id': line_search.line_id.partner_id.id,
                            'history_id': room_name.id,
                            'booking_id': line_search.line_id.id,
                            'state': 'done',
                            'category_id': room_name.categ_id.id,
                            'name': room_name.name,
                            'check_in': vals.get('check_in', line_obj.checkin_date),
                            'check_in_date': vals.get('check_in', line_obj.checkin_date),
                            'check_out': vals.get('checkout_date', line_obj.checkout_date),
                            'check_out_date': vals.get('checkout_date', line_obj.checkout_date),
                            'product_id': vals.get('product_id', line_obj.product_id.id),
                        }
                        # Create a new room booking history record
                        self.env['hotel.room.booking.history'].create(value_history)

            # Update existing booking history records for each room in room_number
            for room in line_search.room_number:
                room_name_K = self.env['hotel.room'].search(
                    [('product_id', '=', room.id)]
                )
                for history_k in room_name_K.room_folio_ids:
                    if (history_k.booking_id.id == line_search.line_id.id
                            and line_obj.checkin_date == history_k.check_in
                            and line_obj.checkout_date == history_k.check_out):

                        # Update check-out date if changed
                        if 'checkout_date' in vals:
                            history_k.write({
                                'check_out': vals['checkout_date'],
                                'check_out_date': vals['checkout_date'],
                        ...
                        # Update check-in date if changed
                        if 'checkin_date' in vals:
                            history_k.write({
            ...
            # Update fields in hotel.reservation.line if they exist in vals
            if line_search:
                updates = {}
                if 'checkout_date' in vals:
                    updates['checkout'] = vals['checkout_date']
                if 'checkin_date' in vals:
                    updates['checkin'] = vals['checkin_date']
                if 'categ_id' in vals:
                    updates['categ_id'] = vals['categ_id']
                if 'product_id' in vals:
                    updates['room_number'] = [(6, 0, [vals['product_id']])]
                if 'discount' in vals:
                    updates['discount'] = vals['discount']
                if 'price_unit' in vals:
                    updates.update({
                        'sub_total1': vals['price_unit'],
                        'price': vals['price_unit'],
                ...
                if 'product_uom_qty' in vals:
                    updates['number_of_days'] = vals['product_uom_qty']
                    line_search.count_price()  # Recalculate price if quantity changed
                line_search.write(updates)

        # Ensure price_unit is set in vals if it was not initially present
