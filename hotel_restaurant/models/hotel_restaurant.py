...
class hotel_restaurant_order(models.Model):
    ...
    manufacturing_count = fields.Integer(compute="_compute_manufacturing", string='Manufacturing Count', copy=False, default=0,
                                   readonly=True)

    manufacturing_ids = fields.Many2many('mrp.production', compute="_compute_manufacturing", string='Manufacturing', copy=False,
                                   readonly=True)
    ...
    @api.depends('manufacturing_count')
    def _compute_manufacturing(self):
        for order in self:
            if self.order_no:
                manufacturing = self.env['mrp.production'].search([('origin', "=", self.order_no)])
                order.manufacturing_ids = manufacturing
                order.manufacturing_count = len(manufacturing)
            else:
    ...
    def action_manufacturing_view(self):
        manufacturing = self.mapped('manufacturing_ids')
        action = self.env.ref('mrp.mrp_production_action').sudo().read()[0]
        if len(manufacturing) > 1:
            action['domain'] = [('id', 'in', manufacturing.ids)]
        elif len(manufacturing) == 1:
            form_view = [(self.env.ref('mrp.mrp_production_form_view').id, 'form')]
            if 'views' in action:
                action['views'] = form_view + [(state, view) for state, view in action['views'] if view != 'form']
            else:
                action['views'] = form_view
            action['res_id'] = manufacturing.id
        else:
            action = {'type': 'ir.actions.act_window_close'}
        return action
    ...
    def create_invoice(self):
        for obj in self:
            ...
            type = 'out_invoice'
            print("22222222222222222222222222222",obj,obj.room_no)
            if not obj.room_no:
                inv = {
                ...
                print("111111111111111111111111111111111111",inv_id)
                todo = []
