...
class hotel_reservation(models.Model):
    ...
    def _get_report_base_filename(self):
        return "Hotel Reservation Report"

    def _get_report_base_filename1(self):
        return "Hotel Reservation Checking List"

    def _get_report_base_filename2(self):
        return "Hotel Reservation Checkout List"

    def _get_report_base_filename3(self):
        return "Room Used Maximum"
    ...
    def state_scheduler(self):
        ...
        for room_id in room_ids:
            ...
            for room_folio_id in room_folio_ids:
                ...
                if room_folio_id.check_out.strftime('%Y-%m-%d') < current_date.strftime(
                        '%Y-%m-%d') and room_folio_id.booking_id.state in ['draft', 'confirm']:
    ...
    def write(self, vals):
        ...
        for res in self:
            ...
            if res.reservation_line:
                for line in res.reservation_line:
                    for room in line.room_number:  # Loop through each room in room_number
                        no_of_adults_room += room.max_adult
                        no_of_child_room += room.max_child
                _logger.info('\n\nReservation: {}\nTotal Adults: {}\nTotal Children: {}\n\n'.format(
                    res.name, no_of_adults_room, no_of_child_room
                ))
        return ret
        ...
        for val in vals:
            ...
            if res:
                ...
                if res.reservation_line:
                    for line in res.reservation_line:
                        if line:
                            # Iterate over each room in room_number and sum up max_adult and max_child
    ...
    def _count_total_rooms(self):
        for order in self:
            count = 0
            for line in order.reservation_line.room_number:
                count += 1
            order.update({
                'number_of_rooms': count,
            })
    ...
    def update_prices(self):
        ...
        for line in self.reservation_line:
            ...
            for room in line.room_number:
                # Set context for each room with quantity and date
                product = room.with_context(
                    quantity=line.number_of_days,
                    date=line.checkin,
                    pricelist=self.pricelist_id.id,
                )
                ...
                # Calculate price considering taxes
                price_unit = self.env['account.tax']._fix_tax_included_price_company(
                    line._get_display_price(product), room.taxes_id, line.taxes_id, line.company_id)
                print(price_unit,"4444444444444444444444")

                if self.pricelist_id.discount_policy == 'without_discount' and price_unit:
                    # Calculate discount for the room
                    discount = max(0, (price_unit - product.price) * 100 / price_unit)
                else:
                    discount = 0

                # Accumulate prices and discounts for all rooms in this line
                price_unit_total += price_unit
                discount_total += discount

            # Average out discount if multiple rooms are involved
            discount_avg = discount_total / len(line.room_number) if line.room_number else 0

            # Add the accumulated price and average discount to update list
            lines_to_update.append(
                (1, line.id, {'price': price_unit_total, 'discount': discount_avg})
            )

        # Apply updates to reservation lines
        self.update({'reservation_line': lines_to_update})
        self.show_update_pricelist = False
        self.message_post(body=_(
            "Product prices have been recomputed according to pricelist <b>%s<b> " % self.pricelist_id.display_name))
    ...
    # @api.multi
    @api.depends('pricelist_id', 'reservation_line.sub_total1')
    ...
    hotel_folio_id = fields.Many2one('hotel.folio', string="Folio", compute="_compute_folio_state", store=True)
    ...
    untaxed_amt = fields.Float(
        compute="_compute_amounts", string="Untaxed Amount")
    amount_untaxed = fields.Monetary(string="Untaxed Amount", store=True, tracking=5)
    ...
    new_currency_id = fields.Many2one('res.currency', string='Currency', related='company_id.currency_id', )
    ...
    '''def action_update_prices(self):
        ...
        print("dddddddddddddddddddd",pricelist_amount)
        for line in self.reservation_line:
            ...
            print("ddddddddddddddddddddddddddddddd",pricev)
    ...
    #     currency_rate = self.pricelist_id.currency_id.rate
    #     print('Currency Rate:', currency_rate)
    #     print('Original untaxed amount:', self.untaxed_amt)
    #
    #     amount = self.untaxed_amt * currency_rate
    #
    #     self.amount_untaxed = amount
    #     self.untaxed_amt = amount
    #
    #     # Update the sub_total1 field based on the pricelist's currency rate
    #     self.reservation_line.count_sub_total1()
    #
    #     print('Updated untaxed amount:', amount)
    #     self.update({
    #         'update_price': True,
    #     })
    ...
    def _compute_amounts(self):
        for obj in self:
            ...
            if obj.pricelist_id and obj.pricelist_id.currency_id:
                currency = obj.pricelist_id.currency_id
            elif obj.country_id and obj.country_id.currency_id:
                currency = obj.country_id.currency_id
            else:
                currency = self.env.user.company_id.currency_id

            # obj.currency_id = currency

            obj.amount_untaxed = currency.round(total)
            obj.untaxed_amt = currency.round(total)
    ...
    def cancel_reservation(self):
        hotel_reservation.cancelation_sms(self)
        for reservation in self:
    ...
    def cancelation_sms(self, max_content_length=None):
        base_url = "https://login.febnosms.com/api/api_http.php?"
        message_text = f"Hi {self.partner_id.name}, your reservation at AlHayat Group Hotel is cancelled!"
        mobile_number = self.phone
        user_id = "hayat"
        password = "jc1Ddaw6Gp"
        senderid = 'SAM'

        api_url = f"{base_url}username={user_id}&password={password}&senderid={senderid}&to={mobile_number}&text={message_text}"
        try:
            response = requests.post(api_url)
            response.raise_for_status()
            content_snippet = response.content[:max_content_length] if response.content else ""
            _logger.info(f"SMS notification sent successfully (response content: {content_snippet}). Room: {self.name}")
            _logger.info("SMS notification sent successfully for room %s.", self.name)
        except requests.exceptions.RequestException as error:
            _logger.error("Error sending SMS notification: %s", error)
    ...
    #     hotel_reservation.send_sms(self)
    #     for reservation in self:
        ...
        for reservation in self:
            if reservation.source == 'internal_reservation':
                if not reservation.reservation_line:
                    raise UserError(_('Please Select Room'))
            if reservation.total_cost1 < reservation.deposit_cost2:
                raise ValidationError("Advance Amount Should not be greater than Total Reservation Cost.")
            if not reservation.reservation_line:
                raise ValidationError("Reservation line Details are missing.")

            for line in reservation.reservation_line:
                # Handle multiple room_number records
                room_line_ids = self.env['hotel.room'].search(
                    [('product_id', 'in', line.room_number.ids)])  # Updated to handle multiple room numbers
                for room_line_id in room_line_ids:
                    # Housekeeping validation
                    housekeeping_rooms = self.env['hotel.housekeeping'].search([
                        ('room_no', '=', room_line_id.product_id.id), ('state', '=', 'dirty')])
                    for housekeeping_room in housekeeping_rooms:
                        house_current_date = fields.Date.from_string(housekeeping_room.current_date)
                        house_end_date = fields.Date.from_string(housekeeping_room.end_date)
                        start_reser = fields.Date.from_string(line.checkin)
                        end_reser = fields.Date.from_string(line.checkout)
                        if (house_current_date <= start_reser <= house_end_date) or \
                                (house_current_date <= end_reser <= house_end_date) or \
                                (start_reser < house_current_date and end_reser > house_end_date):
                            raise ValidationError(
                                "Room %s is not clean for the reservation period!" % room_line_id.name)

                    # Room booking history validation
                    for history in room_line_id.room_folio_ids:
                        if history.state == 'done':
                            if (history.check_in <= line.checkin < history.check_out) or \
                                    (history.check_in < line.checkout <= history.check_out) or \
                                    (line.checkin < history.check_in and line.checkout >= history.check_out):
                                if reservation.id != history.booking_id.id:
                                    raise UserError("Room %s is booked in this reservation period!" % room_line_id.name)

                # Create room booking history
                for room in line.room_number:
                    room_line_id = self.env['hotel.room'].search(
                        [('product_id', '=', room.id)], limit=1)
                    self.env['hotel.room.booking.history'].create({
                        'partner_id': reservation.partner_id.id,
                        'check_in': line.checkin,
                        'check_out': line.checkout,
                        'history_id': room_line_id.id,
    ...
    #     # hotel_reservation.checkin_sms(self)
    #     for record in self:
    #         record.checkin_sms()
    #     for reservation in self:
    #         room_his_id_search = self.env['hotel.room.booking.history'].search(
    ...
    #             room_line_id = self.env['hotel.room'].search(
    #                 [('product_id', '=', line.room_number.id)])
    #             print(room_line_id,"00000000000000000000000")
    #             ###############################################by pornima######
    #             housekeeping_room = self.env['hotel.housekeeping'].search(
    #                 [('room_no', '=', room_line_id.product_id.id), ('state', '=', 'dirty')])
    ...
    # def checkin_sms(self, max_content_length=None):
    #     for record in self:
    #         room_numbers = ', '.join(line.room_number.name for line in record.reservation_line)
    #         wifi = record.shop_id.wifi
    #
    #         base_url = "https://login.febnosms.com/api/api_http.php?"
    #         message_text = f"Hi {record.partner_id.name}, Welcome to AlHayat Group Hotel! Room: {room_numbers}, Wi-Fi Password: '{wifi}'. Enjoy your stay!"
    ...
    #             content_snippet = response.content[:max_content_length] if response.content else ""
    #             _logger.info(
    #                 f"SMS notification sent successfully (response content: {content_snippet}). Room: {record.name}")
    #             _logger.info("SMS notification sent successfully for room %s.", record.name)
    #         except requests.exceptions.RequestException as error:
    #             _logger.error("Error sending SMS notification: %s", error)
    ...
    #
    #     for reservation in self:
    #         # Unlink previous booking history for the reservation
    #         room_his_id_search = self.env['hotel.room.booking.history'].search(
    #             [('booking_id', '=', reservation.id)])
    #         room_his_id_search.unlink()
    #
    ...
    #             raise ValidationError("Reservation line Details are missing.")
    #
    #         # Iterate over each reservation line
    #         for line in reservation.reservation_line:
    #             for room in line.room_number:
    #                 room_line_id = self.env['hotel.room'].search(
    #                     [('product_id', '=', room.id)], limit=1)
    #                 print(room_line_id,"ooooooooooooooooooooo")# Ensure singleton with limit=1
    #
    #                 # Housekeeping check
    #                 housekeeping_room = self.env['hotel.housekeeping'].search(
    #                     [('room_no', '=', room_line_id.product_id.id), ('state', '=', 'dirty')])
    #
    #                 if housekeeping_room:
    #                     for house in housekeeping_room:
    ...
    #                         start_reser = line.checkin.date()
    #                         end_reser = line.checkout.date()
    #
    #                         if (house_current_date <= start_reser <= house_end_date) or \
    #                                 (house_current_date <= end_reser <= house_end_date) or \
    #                                 (start_reser < house_current_date and end_reser > house_end_date):
    ...
    #                                 if reservation.id != history.booking_id.id:
    #                                     raise UserError(
    #                                         "Room %s is already booked during this reservation period!" % room_line_id.name)
    #
    #         # Create the folio
    #         folio = self.env['hotel.folio'].create({
    #             'payment_move_ids': reservation.payment_move_ids.ids,
    ...
    #
    #         # Create room lines in the folio for each room in room_number
    #         folio_lines_to_create = []  # List to collect folio line data
    #
    #         # Loop through each reservation line
    #         for line in reservation.reservation_line:
    #             # Loop through each room associated with the reservation line
    #             for room in line.room_number:
    #                 print(room, "22222222222222222222")
    #                 room_line_id = self.env['hotel.room'].search([('product_id', '=', room.id)], limit=1)
    #                 print(room_line_id, "3333333333333333333")
    #                 print(f"Room ID: {room.id}, UoM ID: {room.uom_id.id}")
    #
    #                 # Create a booking history record for each room in room_number
    #                 self.env['hotel.room.booking.history'].create({
    #                     'partner_id': reservation.partner_id.id,
    #                     'check_in': line.checkin,
    #                     'check_out': line.checkout,
    ...
    #
    #                 # Create a folio line for each room
    #                 try:
    #                     folio_line_record = self.env["hotel_folio.line"].create({
    #                         'folio_id': folio.id,
    #                         'product_id': room.id,  # Ensure the correct product ID is used
    #                         'product_uom': room.uom_id.id,
    #                         'price_unit': line.price,  # Make sure this is the right price for the room
    ...
    #                         'hotel_reservation_line_id': line.id,
    #                     })
    #                     print(f"Folio Line Record Created: {folio_line_record.id}")
    #                 except Exception as e:
    #                     print(f"Error creating folio line record: {str(e)}")
    #
    #                 print("ccccccccccccccccccccccccccc")
    #
    #         # Create all folio lines in one go
    #         if folio_lines_to_create:
    #             created_folio_lines = self.env["hotel_folio.line"].create(folio_lines_to_create)
    #             print(f"Created {len(created_folio_lines)} folio line records.")
    #
    ...
    #         reservation.reservation_line.write({'checkin': line.checkin})
    ...
    def create_folio(self):
        ...
        for reservation in self:
            ...
            # Create room lines in the folio for each room in room_number
            for line in reservation.reservation_line:
                num_rooms = len(line.room_number)
                if num_rooms == 0:
                    continue  # Skip if no rooms are in line

                for room in line.room_number:
                    room_line_id = self.env['hotel.room'].search([('product_id', '=', room.id)], limit=1)
                    # room_price = room.lst_price  # Get price per room from product's list price
                    room_price = 0
                    if line.line_id.meal_id:
                        room_price = room.room_charge + room.meal_charge
                    else:
                        room_price += room.room_charge

                    # Calculate price_subtotal as price per room times number of days
                    price_subtotal = room_price * line.number_of_days
                    # Create a booking history record for each room in room_number
                    self.env['hotel.room.booking.history'].create({
                        'partner_id': reservation.partner_id.id,
                        'check_in': line.checkin,
    ...
    def checkin_sms(self, max_content_length=None):
        for record in self:
            # Create a list of room numbers from the Many2many field
            room_numbers = ', '.join(
                room.name for line in record.reservation_line for room in line.room_number
            )
            wifi = record.shop_id.wifi
    ...
    def done(self):
        print("tettttttttttttttttttttttt")
    ...
    def send_sms(self, max_content_length=None):
        base_url = "https://login.febnosms.com/api/api_http.php?"
        message_text = f"Hi {self.partner_id.name}, your reservation at AlHayat Group Hotel is confirmed! See you soon!"
        mobile_number = self.phone
        user_id = "hayat"
        password = "jc1Ddaw6Gp"
        senderid = 'SAM'
...
class hotel_reservation_line(models.Model):
    ...
    @api.depends('number_of_days', 'discount', 'price', 'taxes_id', 'line_id.pricelist_id')
    def count_sub_total1(self):
        print("subbbbbbbbbbbbbbbbbbbbbbbbbbbb")
        for line in self:
            tax_amount = 0
            ...
            pricelist_amount = line.line_id.pricelist_id.currency_id.rate
            ...
            if line.line_id:
                line.update({
                    ...
                    # 'price': line.price * pricelist_amount,
                })
            # else:
            #     line.update({
            #         'sub_total1': taxes['total_excluded'],
    ...
    room_number = fields.Many2many(
        'product.product',
        string='Room Numbers',
        domain="[('isroom', '=', True), ('categ_id', '=', categ_id)]",
        help="Will list out all the rooms that belong to the selected shop.",
        required=True
    )
    ...
    def _get_display_price(self, product):
        # TO DO: move me in master/saas-16 on sale.
        ...
        if self.item_qty:
            if self.o_l:
                ...
                price_value = self.order_l.pricelist_id._price_get(
                    temp.product_id.id, self.item_qty, uom=temp.uom_id.id, date=orderl_date)[
