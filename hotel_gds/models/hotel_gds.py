...
class hotel_reservation(models.Model):
    ...
    #     [
    #     # ('no_meal', 'No Meal Included'),
    #      ('cont_break', 'Continental Breakfast'),
    #      ('buff_break', 'Buffet Breakfast'),
    #      # ('half_board', 'Half-board'),
    #      # ('full_board', 'Full-board')
    #     ], 'Meal Type', readonly=True,
    ...
    meal_id = fields.Selection(
        ...
        readonly=True,
        states={'draft': [('readonly', False)]},
        default='cont_break'  # Setting the default to "Continental Breakfast"
    )
    ...
    def confirmed_reservation(self):
        ...
        for line in self.reservation_line:
            for room in line.room_number:
                self.env['product.product'].search([('id', '=', room.id)])
        return so
...
class hotel_reservation_line(models.Model):
    ...
    def onchange_room_id(self):
        ...
        # Check if room_number contains multiple records
        if self.room_number:
            for product_browse in self.room_number:  # Loop through each room
                product_id = product_browse.id
                price = product_browse.lst_price
                if price is False:
                    raise UserError("Couldn't find a pricelist line matching this product!")
                ...
                if pricelist:
                    price = self.env['product.pricelist'].with_context(ctx)._price_get(
                        product_browse, self.number_of_days)[pricelist]

                v['price'] = price
                tax_ids = [tax_line.id for tax_line in product_browse.taxes_id]
                v['taxes_id'] = [(6, 0, tax_ids)]
                v['checkin'] = self.checkin
                v['checkout'] = self.checkout
                ...
                if config_browse and self.line_id.source != 'through_gds':
                    for record in config_browse:
                        config_start_date = datetime.strptime(str(record.name), '%Y-%m-%d %H:%M:%S')
                        config_end_date = datetime.strptime(str(record.to_date), '%Y-%m-%d %H:%M:%S')
                        ...
                        if (config_start_date <= reservation_start_date < config_end_date) or (
                                ...
                                reservation_end_date > config_end_date)):
                            for line in record.line_ids:
                                room_list = [room.id for room in line.room_number]
                                if product_browse.id in room_list:
                                    raise UserError("Room is reserved in this period for GDS reservation!")
                ...
                if housekeeping_room:
                    for house in housekeeping_room:
                        house_current_date = datetime.combine(house.current_date, datetime.min.time())
                        house_end_date = datetime.combine(house.end_date, datetime.min.time())
                        ...
                        if (((start_reser < house_current_date) and (end_reser > house_end_date)) or (
                                house_current_date <= start_reser < house_end_date) or (
                                    house_current_date < end_reser <= house_end_date)) and (house.state == 'dirty'):
                            raise UserError(f"Warning! Room {room_line_id.name} is not clean for reservation period!")
                    ...
                    for history in room_line_id.room_folio_ids:
                        if history.state == 'done':
                            ...
                            if (history_start_date <= reservation_start_date < history_end_date) or (
                                    ...
                                    reservation_end_date > history_end_date)):
                                if not (self.line_id.id == history.booking_id.id):
                                    raise UserError(f"Room {room_line_id.name} is booked in this reservation period!")
